...
export default function(api: IApi, options: GlobalOptions) {
  ...
  const { master: masterOpts, slave: slaveOpts, shouldModifyRuntimePublicPath = true } = options || {};
  ...
  if (masterOpts) {
    ...
    api.registerPlugin({
      id: 'qiankun-slave',
      apply: slave,
      opts: { ...slaveOpts, shouldModifyRuntimePublicPath, registerRuntimeKeyInIndex: true },
    });
  }
}
